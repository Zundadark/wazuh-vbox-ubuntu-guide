...
1. В настройках сети ВМ VirtualBox выберите **Сетевой мост (Bridge)**.
2. В консоли Ubuntu узнайте IP-адрес сервера командой: `ip a`
3. Убедитесь, что SSH-сервер активен: `sudo systemctl status ssh` (если нет, установите: `sudo apt install openssh-server -y`).
4. Откройте **Windows Terminal** на основном компьютере и подключитесь одной командой:

```bash
ssh имя_пользователя@IP_адрес_сервера
```
*При первом подключении введите `yes`, а затем пароль от Ubuntu*

## 🔑 Лайфхак: Как вспомнить забытый пароль от Wazuh

Если вы забыли пароль от веб-интерфейса `admin`, его можно вытащить прямо из недр сервера. Подключитесь по SSH и выполните команду:

```bash
sudo find / -name "wazuh-passwords.txt" -exec cat {} \; 2>/dev/null
```
*Команда найдет скрытый файл инсталлятора и выведет логин и пароль на экран, проигнорировав системные ошибки.*
